...
smsConsentAPI = SmsReader "SMS_CONSENT" (runExceptT getNextSms)
  ...
  getNextSms = do
    ...
    _ <- if (length sms) == 0
              then pure unit
              else liftEffect $ trackEvent "no_of_sms_consent" (show $ length sms)
    if length sms < 1
      then getNextSms
      else pure sms
...
clipboard = SmsReader "CLIPBOARD" (runExceptT getNextSms)
  where
    ...
    getNextSms = do
      ...
      _ <- if (length sms) == 0
              then pure unit
              else liftEffect $ trackEvent "no_of_sms_clipboard" (show $ length sms)
      if length sms < 1
        then getNextSms
        else pure sms
...
getOtpListener readers = do
  ...
  where
    ...
    pushLogs rule (ReceivedSms sms reader)= do
